...
/*   Updated: 2023/06/12 16:18:37 by gbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			read_client_input(t_client *client)
{
	...
	{
		cl_log(CL_LOG_DEBUG, client, "%d", ret);
		i = 0;
...
void		write_to_client(t_client *client)
{
	...
	if (ret == ERR)
		cl_log(CL_LOG_ERROR, client, "write");
...
void		read_from_server(t_client *client)
{
	...
	cl_log(CL_LOG_DEBUG, client, "Reading from server %d and received %ld", read_b, ret);
	if (ret < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
	{
